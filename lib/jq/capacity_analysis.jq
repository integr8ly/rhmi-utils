...
[[.[] | {"key": .id, "value": (.result.items? // .result)}] | from_entries | . as {pods: $pods, volumes: $vols, "pod-metrics": $podMetrics, nodes: $nodes} |
  ...
  $pod + {
    ...
    cpu_req: i8::normalize_cpu(.requests?.cpu?),
    cpu_lim: i8::normalize_cpu(.limits?.cpu?),
    cpu_real: i8::normalize_cpu(.usage?.cpu?),
    mem_req: (if .requests?.memory? then i8::mem_to_bytes(.requests?.memory?) else null end),
    mem_lim: (if .limits?.memory? then i8::mem_to_bytes(.limits?.memory?) else null end),
    mem_real: (if .usage?.memory? then i8::mem_to_bytes(.usage?.memory?) else null end)
  }
] | unique
